    ...
    def _analyze_market_conditions(assets):
        try:
            ...
            result = {
                ...
                "fear_greed_classification": market_data.get("fear_greed_classification"),
            ...
            if (
                ...
                for symbol in asset_symbols:
                    try:
                        ...
                        try:
                            ...
                            price_change_1h = data.get("price_change_percentage_1h_in_currency", 0)
                            ...
                            strong_bull_24h = 5 * volatility_factor * threshold_multiplier
                            strong_bull_7d = 10 * volatility_factor * threshold_multiplier
                            ...
                            # Short-term (1-3 days)
                            if price_change_24h > strong_bull_24h and price_change_1h > 0:
                                short_term_trend = "strong_bullish"
                            elif price_change_24h > bull_24h:
                                short_term_trend = "bullish"
                            elif price_change_24h < -strong_bull_24h and price_change_1h < 0:
                            ...
                            if short_term_trend == "strong_bullish" and medium_term_trend in ["strong_bullish", "bullish"]:
                                asset_trend = "strong_bullish"
                            elif short_term_trend == "strong_bearish" and medium_term_trend in ["strong_bearish", "bearish"]:
                                asset_trend = "strong_bearish"
                            elif short_term_trend in ["bullish", "strong_bullish"] and medium_term_trend != "strong_bearish":
                                asset_trend = "bullish"
                            elif short_term_trend in ["bearish", "strong_bearish"] and medium_term_trend != "strong_bullish":
                                asset_trend = "bearish"
                            else:
                                asset_trend = "mixed"
                            ...
                            avg_volume = technical_indicators.get("average_volume_10d", volume_24h)
                            ...
                            if asset_trend in ["strong_bullish", "bullish"] and volume_signal == "high":
                                trend_strength = "confirmed"
                            elif asset_trend in ["strong_bearish", "bearish"] and volume_signal == "high":
                                trend_strength = "confirmed"
                            else:
                            ...
                            expected_asset_trends = market_trend_map.get(market_trend, ["mixed"])
                            
                            if asset_trend == "strong_bullish" and market_trend != "bull_market":
                                asset_sentiment = "strongly_outperforming"
                            elif asset_trend == "strong_bearish" and market_trend != "bear_market":
                                asset_sentiment = "strongly_underperforming"
                            elif asset_trend in ["bullish", "strong_bullish"] and market_trend == "bear_market":
                                asset_sentiment = "outperforming"
                            elif asset_trend in ["bearish", "strong_bearish"] and market_trend == "bull_market":
                            ...
                            support_level = technical_indicators.get("support_level", data.get("low_24h", 0) * 0.95)
                            resistance_level = technical_indicators.get("resistance_level", data.get("high_24h", 0) * 1.05)
                            ...
                            asset_specific_analysis[symbol_upper] = {
                                ...
                                "price_changes": {
                                    ...
                                    "7d": price_change_7d,
                                    "30d": data.get("price_change_percentage_30d_in_currency", 0),
                                },
                                "volume": {
                                    "24h": volume_24h,
                                ...
                                "key_levels": {
                                    ...
                                    "atl": data.get("atl", 0),
                                    "atl_change_percentage": data.get("atl_change_percentage", 0),
                                },
                                "trends": {
                                ...
                                "recommendation": get_asset_recommendation(
                                    asset_trend, result["market_condition"], rsi, distance_to_support, 
                                    distance_to_resistance, trend_strength
                                ),
                            }
                ...
                if len(asset_specific_analysis) > 1:
                    try:
                        ...
                        if isinstance(correlations, dict) and not "error" in correlations:
                            avg_correlation = calculate_average_correlation(correlations)
                            result["portfolio_metrics"] = {
                                "average_correlation": avg_correlation,
                                "diversification_score": 100 - (avg_correlation * 100) if avg_correlation <= 1 else 0
                            }
                    except Exception as e:
...
def calculate_support_resistance(prices):
    ...
    resistance_level = min(resistance_candidates) if resistance_candidates else highest_price
        ...
        if isinstance(correlations, dict):
            for symbol2, corr_value in correlations.items():
                if symbol1 != symbol2 and isinstance(corr_value, (int, float)):
                    total_correlation += abs(corr_value)  # Use absolute value for correlation strength
                    pair_count += 1
...
def get_asset_recommendation(asset_trend, market_condition, rsi, distance_to_support, 
                            distance_to_resistance, trend_strength):
...
def generate_market_insights(market_data):
    ...
    market_cap_change = market_data.get("market_metrics", {}).get("market_cap_change_24h")
    ...
    if "asset_analysis" in market_data and isinstance(market_data["asset_analysis"], dict):
        outperformers = []
        underperformers = []
        overbought_assets = []
        oversold_assets = []
        ...
        for symbol, data in market_data["asset_analysis"].items():
            ...
            elif data.get("sentiment") in ["strongly_underperforming", "underperforming"]:
                underperformers.append(symbol)
                
            # Check technical indicators for overbought/oversold conditions
            if "technical_indicators" in data and data["technical_indicators"].get("rsi_signal") == "overbought":
                overbought_assets.append(symbol)
            elif "technical_indicators" in data and data["technical_indicators"].get("rsi_signal") == "oversold":
    ...
    if "asset_correlations" in market_data and isinstance(market_data["asset_correlations"], dict):
        high_correlations = []
        low_correlations = []
    ...
    insights.sort(key=lambda x: importance_rank.get(x.get("importance", "LOW"), 0), reverse=True)
    ...
    def _get_market_opportunities(uid, opp_type):
        try:
            ...
            market_conditions = analyze_market_conditions.invoke(
                {"asset_symbols": ["BTC", "ETH", "BNB", "SOL", "XRP", "ADA", "DOT", "MATIC", "AVAX", "LINK"]}
            )
            ...
            if (
                ...
                market_regime = market_conditions.get("market_regime", {}).get("regime", "neutral")
                ...
                if opportunity_type in ["ALL", "DIP_BUYING"]:
                    for symbol, asset_data in market_conditions["asset_analysis"].items():
                        ...
                        elif price_change_24h < -10 and price_change_7d < -5 and rsi < 40:
                            is_dip_opportunity = True
                            reasoning.append(f"Sharp correction: 24h: {price_change_24h:.1f}%, 7d: {price_change_7d:.1f}% with RSI at {rsi}")
                        
                        # Near strong support level
                        elif distance_to_support < 5 and trend not in ["strong_bearish"] and rsi < 45:
                            is_dip_opportunity = True
                            reasoning.append(f"Price near support level ({distance_to_support:.1f}% away)")
                        ...
                        if is_dip_opportunity:
                            ...
                            elif market_regime in ["optimism", "neutral"] and distance_to_support < 3:
                                opportunity_strength = "HIGH"
                            ...
                            base_allocation = min(total_value * 0.03, 1000)  # Default 3% of portfolio or $1000
                            ...
                            allocation_suggestion = round(base_allocation * allocation_multiplier * category_multiplier, 2)
                            
                            # Entry strategy based on market conditions
                            if risk_level in ["HIGH", "VERY_HIGH"]:
                                entry_strategy = "Gradual entry with 3-4 tranches over 1 week"
                            else:
                                entry_strategy = "Split entry into 2 tranches over 2-3 days"
                            ...
                            opportunities.append({
                                ...
                                "potential_return": get_potential_return_estimate(symbol, asset_data),
                                "time_horizon": get_time_horizon_estimate(symbol, asset_data, market_regime),
                ...
                if opportunity_type in ["ALL", "PROFIT_TAKING"]:
                    for position in portfolio.get("positions_by_asset", []):
                        ...
                        if pnl_percentage > 100:
                            ...
                            reasoning.append(f"Significant profit achieved ({pnl_percentage:.1f}%)")
                            opportunity_strength = "HIGH"
                            exit_strategy = "Consider taking 50-75% profit to secure gains"
                        elif pnl_percentage > 50:
                            is_profit_opportunity = True
                            reasoning.append(f"Healthy profit achieved ({pnl_percentage:.1f}%)")
                            opportunity_strength = "MEDIUM"
                            ...
                            if "technical_indicators" in asset_data:
                                ...
                                if rsi > 70:
                                    is_profit_opportunity = True
                                    reasoning.append(f"Moderate profit ({pnl_percentage:.1f}%) with overbought conditions (RSI: {rsi})")
                                    exit_strategy = "Consider taking 15-25% profit"
                            
                        # Check technical signals for profit taking
                        if "technical_indicators" in asset_data and not is_profit_opportunity:
                            rsi = asset_data["technical_indicators"].get("rsi_14d", 50)
                            if rsi > 75 and pnl_percentage > 15:
                                is_profit_opportunity = True
                                reasoning.append(f"Overbought technical conditions (RSI: {rsi})")
                        ...
                        if "key_levels" in asset_data and not is_profit_opportunity:
                            distance_to_resistance = asset_data["key_levels"].get("distance_to_resistance", 20)
                            if distance_to_resistance < 3 and pnl_percentage > 15:
                                is_profit_opportunity = True
                                reasoning.append(f"Price approaching resistance ({distance_to_resistance:.1f}% away)")
                                exit_strategy = "Consider taking 15-25% profit near resistance"
                        ...
                        if is_profit_opportunity:
                            ...
                            if market_regime == "euphoria":
                                if opportunity_strength != "HIGH":
                                    opportunity_strength = "HIGH"
                                reasoning.append("Market showing signs of excessive optimism")
                                exit_strategy = exit_strategy.replace("25-50%", "40-60%").replace("15-25%", "30-40%")
                            
                            # In fear/capitulation, be more selective with profit taking
                            elif market_regime in ["fear", "capitulation"] and opportunity_strength != "HIGH":
                            ...
                            if is_profit_opportunity:
                                opportunities.append({
                                    ...
                                    "market_context": {
                                        "asset_trend": asset_data.get("trends", {}).get("overall", "unknown"),
                    ...
                    if isinstance(current_allocation, list):
                        ...
                        if total_items > 0:
                            ...
                            for item in current_allocation:
                                ...
                                # Get market data for this asset if available
                                asset_data = market_conditions["asset_analysis"].get(asset_symbol.upper(), {})
                                asset_trend = asset_data.get("trends", {}).get("overall", "unknown")
                                ...
                                if percentage > 40:
                                    ...
                                    reasoning.append(f"Extremely concentrated position ({percentage:.1f}% of portfolio)")
                                    target_allocation = 25
                                # Handle significant concentration
                                elif percentage > 30:
                                    is_rebalance_needed = True
                                    reasoning.append(f"Highly concentrated position ({percentage:.1f}% of portfolio)")
                                    target_allocation = 20
                                # Handle moderate concentration with negative trend
                                elif percentage > 20 and asset_trend in ["bearish", "strong_bearish"]:
                                    is_rebalance_needed = True
                                    reasoning.append(f"Concentrated position ({percentage:.1f}%) with bearish trend")
                                    target_allocation = 15
                                    
                                # Add context based on market regime
                                if is_rebalance_needed:
                                    if market_regime in ["euphoria", "distribution"]:
                                        reasoning.append("Market showing signs of excessive optimism or distribution")
                                        target_allocation -= 5  # Reduce target allocation further
                                    
                                    # Calculate amount to rebalance
                                    portfolio_value = portfolio.get("total_value", 0)
                                    current_value = (percentage / 100) * portfolio_value
                                    target_value = (target_allocation / 100) * portfolio_value
                                    rebalance_amount = current_value - target_value
                                    
                                    # Risk level based on the concentration and market conditions
                                    risk_level = "MEDIUM"
                                    if percentage > 40 or market_regime in ["euphoria", "distribution"]:
                                        risk_level = "HIGH"
                                    elif percentage < 25:
                                        risk_level = "LOW"
                            ...
                            if "asset_correlations" in market_conditions and total_items >= 3:
                                avg_correlation = market_conditions.get("portfolio_metrics", {}).get("average_correlation", 0.5)
                                
                                if avg_correlation > 0.7:
                                    # Find highly correlated pairs to suggest diversification
                                    high_correlation_pairs = []
                                    
                                    for symbol1, correlations in market_conditions["asset_correlations"].items():
                                        if isinstance(correlations, dict):
                                            for symbol2, correlation in correlations.items():
                                                if symbol1 != symbol2 and correlation > 0.8:
                                    ...
                                    if high_correlation_pairs:
                                        ...
                                        top_pairs = high_correlation_pairs[:2]  # Top 2 most correlated pairs
                                        
                                        for symbol1, symbol2, correlation in top_pairs:
                                            # Find which one to reduce based on performance or allocation
                                            symbol1_allocation = next((item.get("percentage", 0) for item in current_allocation if item.get("group", "") == symbol1), 0)
                                            symbol2_allocation = next((item.get("percentage", 0) for item in current_allocation if item.get("group", "") == symbol2), 0)
                                            
                                            # Default to reducing the higher allocated one
                                            reduce_symbol = symbol1 if symbol1_allocation > symbol2_allocation else symbol2
                                            keep_symbol = symbol2 if reduce_symbol == symbol1 else symbol1
                                            ...
                                            opportunities.append({
                                                ...
                                                "alternative_suggestions": get_diversification_alternatives(market_conditions)
                                            })
                ...
                if opportunity_type in ["ALL", "YIELD"]:
                    ...
                    stablecoin_positions = [
                        p for p in portfolio.get("positions_by_asset", [])
                        if any(
                            stable in p.get("symbol", "").upper()
                            for stable in ["USDT", "USDC", "DAI", "BUSD", "TUSD", "USDP", "USDD", "FRAX"]
                        )
                    ]
                    
                    stablecoin_value = sum(p.get("total_value", 0) for p in stablecoin_positions)
                    stablecoin_breakdown = {p.get("symbol", "Unknown"): p.get("total_value", 0) for p in stablecoin_positions}
                    ...
                    if stablecoin_value > 500:  # Minimum threshold for meaningful yield strategies
                        ...
                        yield_opportunity = {
                            ...
                            "risk_level": "LOW" if market_regime not in ["fear", "capitulation"] else "VERY_LOW",
                            "platforms": []
                        }
                        ...
                        for platform in current_yield_rates:
                            ...
                            risk_adjusted_platforms.append((platform, risk_adjusted_yield))
                        
                        # Sort by risk-adjusted yield
                        risk_adjusted_platforms.sort(key=lambda x: x[1], reverse=True)
                        
                        # Add top platforms
                        for platform, _ in risk_adjusted_platforms[:5]:  # Top 5 platforms
                        ...
                        if market_regime in ["fear", "capitulation"]:
                            ...
                            yield_opportunity["recommended_allocation"] = stablecoin_value * 0.5
                            yield_opportunity["allocation_strategy"] = "Conservative deployment due to market uncertainty"
                        else:
                            # More aggressive in normal/bullish markets
                            yield_opportunity["recommended_allocation"] = stablecoin_value * 0.7
                            yield_opportunity["allocation_strategy"] = "Standard deployment across recommended platforms"
                        
                        # Add action steps
                        yield_opportunity["action"] = "Deploy capital to DeFi protocols for passive income"
                ...
                if opportunity_type in ["ALL", "TRENDING"]:
                    ...
                    for symbol, asset_data in market_conditions["asset_analysis"].items():
                        ...
                        if trend in ["strong_bullish", "bullish"] and volume_signal == "high":
                            is_trending = True
                        ...
                        # Significant recent momentum
                            ...
                            is_trending = True
                            reasoning.append(f"Strong momentum: 24h: +{price_change_24h:.1f}%, 7d: +{price_change_7d:.1f}%")
                        ...
                        # High relative volume indicating increased interest
                            ...
                            is_trending = True
                            reasoning.append(f"High trading volume relative to market cap with positive price action")
                        ...
                        if is_trending:
                            ...
                            base_allocation = min(total_value * 0.02, 500)  # Default 2% of portfolio or $500
                            ...
                            # Final allocation
                            allocation_suggestion = round(base_allocation * regime_multiplier, 2)
                            ...
                            trending_assets.append({
                                ...
                                "take_profit_levels": [
                                    {"level": 1, "percentage": "15-20%", "action": "Take 20% of position"},
                                    {"level": 2, "percentage": "40-50%", "action": "Take another 30% of position"},
                                    {"level": 3, "percentage": "100%+", "action": "Consider full exit or trailing stop"}
                                ]
                            })
                    ...
                    if trending_assets:
                        ...
                        def trend_sort_key(item):
                            strength_score = {"HIGH": 3, "MEDIUM": 2, "LOW": 1}.get(item.get("trend_strength", "MEDIUM"), 0)
                            momentum = item.get("momentum_metrics", {}).get("price_change_7d", 0)
               ...
               # Add market context to response
                market_context = {
                    "overall_trend": market_conditions.get("market_condition", "unknown"),
                    "market_regime": market_regime,
                    "fear_greed_index": fear_greed_index,
                    "risk_level": risk_level,
                    "btc_dominance": market_conditions.get("market_metrics", {}).get("btc_dominance", 0),
                    "market_insights": summarize_market_insights(market_conditions.get("market_insights", []))
                }
                
                # Add portfolio context to response
                portfolio_context = {
                    "total_value": portfolio.get("total_value", 0),
                    "cash_available": stablecoin_value if "stablecoin_value" in locals() else 0,
                    "current_positions": len(portfolio.get("positions_by_asset", [])),
                    "average_correlation": market_conditions.get("portfolio_metrics", {}).get("average_correlation", 0),
                    "diversification_score": market_conditions.get("portfolio_metrics", {}).get("diversification_score", 0)
                }
...
def get_potential_return_estimate(symbol, asset_data):
    ...
    # Final calculations
    low_return = max(base_returns["low"] * trend_multiplier * rsi_multiplier * ath_multiplier, 5)
    mid_return = max(base_returns["mid"] * trend_multiplier * rsi_multiplier * ath_multiplier, 10)
    high_return = max(base_returns["high"] * trend_multiplier * rsi_multiplier * ath_multiplier, 15)
...
def get_time_horizon_estimate(symbol, asset_data, market_regime):
    ...
    base_horizons = {
        "large_cap": {"short": "2-4 weeks", "medium": "1-3 months", "long": "3-6 months"},
        "mid_cap": {"short": "1-3 weeks", "medium": "3-8 weeks", "long": "2-4 months"},
        "small_cap": {"short": "1-2 weeks", "medium": "2-6 weeks", "long": "1-3 months"}
    }.get(asset_category, {"short": "1-3 weeks", "medium": "1-3 months", "long": "3-6 months"})
    
    # Select horizon based on trend and market regime
    if trend in ["strong_bullish", "bullish"] and market_regime in ["optimism", "euphoria"]:
        return base_horizons["short"]  # Shorter timeframes in strong uptrends
    elif trend in ["strong_bearish", "bearish"] or market_regime in ["fear", "capitulation"]:
...
def get_diversification_alternatives(market_conditions):
    ...
    if "asset_correlations" in market_conditions and isinstance(market_conditions["asset_correlations"], dict):
        ...
        for symbol, correlations in market_conditions["asset_correlations"].items():
            if isinstance(correlations, dict):
                ...
                if correlation_values:
                    asset_avg_correlations[symbol] = sum(correlation_values) / len(correlation_values)
        
        # Sort by lowest average correlation
        low_correlation_assets = sorted(asset_avg_correlations.items(), key=lambda x: x[1])[:3]
...
def summarize_market_insights(insights):
    ...
    summary = ""
    if high_importance:
        high_messages = [i.get("message", "").split(".")[0] for i in high_importance[:2]]
    ...
    if len(positive) > len(negative) + len(warnings):
        ...
        summary += "Mixed market signals with balanced positive and negative indicators. "
