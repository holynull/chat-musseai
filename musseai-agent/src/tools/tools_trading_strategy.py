    ...
    try:
        ...
        if len(parts) >= 3:
            ...
            try:
                ...
                logger.info(f"Parsed '{time_str}' to timestamp {timestamp} (UTC: {datetime.datetime.fromtimestamp(timestamp, pytz.UTC)})")
                return timestamp
        ...
        for fmt in formats:
            try:
                ...
                if dt.tzinfo is None:
                    logger.warning(f"No timezone specified in '{time_str}', assuming UTC")
                    dt = dt.replace(tzinfo=pytz.UTC)
...
def generate_and_validate_signal(
    symbol: str, analysis_timeframe: str = "1h", signal_type: str = "day_trading"
) -> str:
    """
    Generate trading signal and automatically validate it with post-signal backtest.

    This is the recommended way to generate signals as it includes automatic validation
    using only market data that occurs AFTER the signal generation time.

    Args:
        symbol (str): Cryptocurrency symbol (e.g., "BTC", "ETH")
        analysis_timeframe (str): Timeframe for analysis ("1m", "5m", "15m", "1h", "4h", "1d")
        signal_type (str): Type of signal ("scalping", "day_trading", "swing_trading")

    Returns:
        str: Complete signal with automatic post-signal validation results
    """
    try:
        logger.info(f"Generating and validating signal for {symbol}")

        # This would integrate with the main signal generation logic
        # and automatically call backtest_trading_signal with the generated signal

        # For now, return instruction for manual process
        return json.dumps(
            {
                "instruction": "Signal generation with auto-validation",
                "process": [
                    "1. Generate signal using standard tools (getLatestQuote, get_indicators, etc.)",
                    "2. Extract signal parameters (direction, entry, stop, target, signal_time)",
                    "3. Automatically call backtest_trading_signal with extracted parameters",
                    "4. Return combined signal + validation results",
                ],
                "note": "This ensures every signal is validated against post-signal market data",
                "symbol": symbol,
        ...
        logger.error(f"Error in generate_and_validate_signal: {e}")
        return json.dumps(
            {
                "error": True,
                "message": f"Failed to generate validated signal: {str(e)}",
            },
            ensure_ascii=False,
            indent=2,
        )
...
def convert_to_utc_time(
    ...
    try:
        ...
        logger.info(f"Successfully converted {local_time} {source_timezone} to {utc_time_str}")
        return json.dumps(result, ensure_ascii=False, indent=2)
