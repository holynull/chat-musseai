...
@tool
def analyze_market_regime(symbols: str, exchange: str, lookback_days: int = 30) -> str:
    """
    Analyzes current market regime using real market data to determine optimal trading strategy type for one or more symbols.

    Evaluates market conditions across multiple dimensions to recommend
    the most suitable trading approach (scalping, day trading, swing trading).
    ...
    Args:
        ...
        lookback_days (int): Days of historical data for analysis (default: 30)

    Returns:
        str: JSON string containing comprehensive market regime analysis with:
             - Real volatility assessment for each symbol
             - Actual trend strength evaluation for each symbol
             - Current liquidity conditions for each symbol
             - Data-driven strategy recommendations
             - Risk level adjustments based on market data
    """
    from utils.api.cryptocompare import (
        calculateVolatilityMetrics,
        analyzeTrendStrength,
        analyzeLiquidityConditions,
    )
    from loggers import logger

    logger.info("analyze_market_regime starting")
    try:
        # Parse symbols
        ...
        # Get liquidity conditions for all symbols at once (outside the loop)
        liquidity_result = json.loads(
            analyzeLiquidityConditions(symbols, exchange, logger)
        )

        # Check if liquidity analysis failed
        if liquidity_result.get("error"):
            return json.dumps(
                {
                    "error": True,
                    "message": "Failed to fetch liquidity data for symbols",
                    "symbols_requested": symbol_list,
                    "details": liquidity_result.get(
                        "message", "Unknown liquidity analysis error"
                    ),
                }
            )
        ...
        for symbol in symbol_list:
            try:
                # Get volatility and trend data for this symbol (liquidity already fetched)
                volatility_result = json.loads(
                    calculateVolatilityMetrics(symbol, lookback_days, logger)
                )
                trend_result = json.loads(
                    analyzeTrendStrength(symbol, lookback_days, logger)
                )

                # Check for errors in data fetching
                if volatility_result.get("error") or trend_result.get("error"):
                    analysis["regime_analysis"][symbol] = {
                        "error": True,
                        "message": f"Failed to fetch volatility or trend data for {symbol}",
                        "symbol": symbol,
                    }
                    continue

                # Extract liquidity data for this symbol from the combined result
                liquidity_analysis = liquidity_result.get("liquidity_analysis", {}).get(
                    symbol, {}
                )
                if liquidity_analysis.get("error"):
                    analysis["regime_analysis"][symbol] = {
                        "error": True,
                        "message": f"No liquidity data available for {symbol}",
                        "symbol": symbol,
                    }
                    continue

                # Extract key metrics
                vol_metrics = volatility_result["volatility_metrics"]
                trend_analysis = trend_result["trend_analysis"]

                # Market regime analysis based on real data
                volatility_level = vol_metrics["volatility_level"]
                volatility_percentile = vol_metrics["volatility_percentile"]
                trend_direction = trend_analysis["overall_assessment"]["primary_trend"]
                trend_strength = trend_analysis["overall_assessment"]["trend_strength"]
                trend_alignment = trend_analysis["trend_alignment"]
                liquidity_level = liquidity_analysis["liquidity_level"]
                spread_percent = liquidity_analysis["spread_percent"]

                # Determine optimal strategy based on market conditions
                def determine_optimal_strategy():
                    # High volatility + good liquidity = Scalping opportunities
                    if (
                        volatility_level == "high"
                        and liquidity_level in ["high", "medium"]
                        and spread_percent < 0.3
                    ):
                        return {
                            "strategy_type": "Scalping",
                            "confidence": "high",
                            "reasoning": f"High volatility ({volatility_percentile:.1f} percentile) with good liquidity enables quick profit opportunities",
                            "timeframe": "1m-5m",
                        }

                    # Strong trend + medium volatility = Day trading
                    elif (
                        ...
                        return {
                            "strategy_type": "Day Trading",
                            "confidence": (
                                "high" if trend_strength == "strong" else "medium"
                            ),
                            "reasoning": f"{trend_strength.title()} {trend_direction} trend with aligned timeframes supports intraday momentum",
                            "timeframe": "15m-1h",
                        }

                    # Stable trend + lower volatility = Swing trading
                    elif trend_direction != "sideways" and volatility_level in [
                        "low",
                        "medium",
                    ]:
                        return {
                            "strategy_type": "Swing Trading",
                            "confidence": "medium",
                            "reasoning": f"Moderate volatility with {trend_direction} bias suitable for longer holds",
                            "timeframe": "4h-1d",
                        }

                    # Sideways market + low volatility = Range trading or wait
                    elif trend_direction == "sideways":
                        return {
                            "strategy_type": "Range Trading",
                            "confidence": "low",
                            "reasoning": "Sideways market with defined support/resistance levels",
                            "timeframe": "1h-4h",
                        }
                    ...
                    # Default fallback
                        ...
                        return {
                            "strategy_type": "Conservative Day Trading",
                            "confidence": "low",
                            "reasoning": "Mixed signals suggest cautious approach",
                            "timeframe": "30m-1h",
                        }

                primary_strategy = determine_optimal_strategy()

                # Generate alternative strategies
                alternative_strategies = []

                if (
                    primary_strategy["strategy_type"] != "Scalping"
                    ...
                    alternative_strategies.append(
                        {
                            "type": "Scalping",
                            "suitability": (
                                "good" if volatility_level == "high" else "fair"
                            ),
                            "conditions": "If volatility increases or during high-volume periods",
                        }
                    )

                if primary_strategy[
                    "strategy_type"
                ] != "Swing Trading" and trend_strength in ["moderate", "strong"]:
                    alternative_strategies.append(
                        {
                            "type": "Swing Trading",
                            "suitability": "good" if trend_alignment else "fair",
                            "conditions": "If trend continues and volatility stabilizes",
                        }
                    )

                # Risk adjustments based on market data
                risk_adjustments = {
                    "position_sizing": (
                        "decrease"
                        if volatility_percentile > 80
                        else "increase" if volatility_percentile < 20 else "maintain"
                    ),
                    "stop_loss_width": (
                        "wider"
                        if vol_metrics["volatility_level"] == "high"
                        else "normal"
                    ),
                    "take_profit": (
                        "conservative"
                        if liquidity_level == "low"
                        else "aggressive" if trend_strength == "strong" else "moderate"
                    ),
                    "max_positions": (
                        "reduce"
                        if volatility_level == "high" or liquidity_level == "low"
                        else "normal"
                    ),
                }

                # Monitoring alerts based on current conditions
                monitoring_alerts = []

                if volatility_percentile > 90:
                    monitoring_alerts.append(
                        "Extreme volatility detected - consider reducing position sizes"
                    )

                if not trend_alignment:
                    monitoring_alerts.append(
                        "Timeframe divergence detected - watch for trend reversal"
                    )

                if liquidity_level == "low":
                    monitoring_alerts.append(
                        "Low liquidity conditions - expect higher slippage"
                    )

                if spread_percent > 0.5:
                    monitoring_alerts.append(
                        "Wide spreads detected - factor in higher trading costs"
                    )

                # Build symbol analysis result
                symbol_analysis = {
                    "symbol": symbol,
                    "market_regime_analysis": {
                        "volatility_assessment": {
                            "current_level": volatility_level,
                            "percentile_rank": volatility_percentile,
                            "annualized_volatility": vol_metrics[
                                "annualized_volatility"
                            ],
                            "trend": vol_metrics["volatility_trend"],
                            "implication": f"{volatility_level.title()} volatility "
                            + (
                                "favors scalping strategies"
                                if volatility_level == "high"
                                else (
                                    "supports swing trading"
                                    if volatility_level == "low"
                                    else "suitable for day trading"
                                )
                            ),
                        },
                        "trend_analysis": {
                            ...
                            "confidence": trend_analysis["overall_assessment"][
                                "confidence"
                            ],
                            "daily_trend_change": trend_analysis["daily_trend"][
                                "trend_change_pct"
                            ],
                            "reliability": (
                                "high"
                                if trend_alignment and trend_strength == "strong"
                                else "moderate"
                            ),
                        },
                        "liquidity_conditions": {
                            ...
                            "slippage_risk": liquidity_analysis["slippage_risk"],
                            "trading_suitability": (
                                "excellent"
                                if liquidity_level == "high"
                                else (
                                    "good" if liquidity_level == "medium" else "limited"
                                )
                            ),
                        },
                        "market_structure": {
                            "regime_type": f"{volatility_level}_volatility_{trend_direction}_trend",
                            "trading_difficulty": (
                                "easy"
                                if (
                    ...
                    "strategy_recommendations": {
                        "primary_recommendation": primary_strategy,
                        "alternative_strategies": alternative_strategies,
                        "market_timing": {
                            "entry_conditions": (
                                "favorable"
                                if (trend_alignment and liquidity_level != "low")
                    ...
                    "market_outlook": {
                        "short_term": f"{trend_analysis['hourly_trend']['direction']} bias with {volatility_level} volatility",
                        "medium_term": (
                            f"{trend_direction} trend expected to continue"
                            if trend_strength == "strong"
                            else "trend may weaken"
                        ),
                        "key_levels_to_watch": "Major support/resistance levels based on recent price action",
                        "regime_stability": (
                            "stable"
                            if (volatility_level != "high" and trend_alignment)
                            else "volatile"
                        ),
                    },
                }
                analysis["regime_analysis"][symbol] = symbol_analysis
                successful_analyses.append(symbol_analysis)

            except Exception as symbol_error:
                logger.error(f"Error analyzing {symbol}: {str(symbol_error)}")
                analysis["regime_analysis"][symbol] = {
                    "error": True,
                    "message": f"Analysis failed for {symbol}: {str(symbol_error)}",
                    "symbol": symbol,
                }

        # Generate overall assessment if multiple symbols
        if len(successful_analyses) > 1:
            # Calculate aggregate metrics
            avg_volatility_percentile = sum(
                a["market_regime_analysis"]["volatility_assessment"]["percentile_rank"]
                for a in successful_analyses
            ) / len(successful_analyses)

            volatility_levels = [
                a["market_regime_analysis"]["volatility_assessment"]["current_level"]
                for a in successful_analyses
            ]
            trend_directions = [
                a["market_regime_analysis"]["trend_analysis"]["direction"]
                for a in successful_analyses
            ]
            liquidity_levels = [
                a["market_regime_analysis"]["liquidity_conditions"]["level"]
                for a in successful_analyses
            ]

            # Determine dominant characteristics
            dominant_volatility = max(
                set(volatility_levels), key=volatility_levels.count
            )
            dominant_trend = max(set(trend_directions), key=trend_directions.count)
            dominant_liquidity = max(set(liquidity_levels), key=liquidity_levels.count)

            # Count strategy recommendations
            strategy_counts = {}
            for analysis_item in successful_analyses:
                strategy = analysis_item["strategy_recommendations"][
                    "primary_recommendation"
                ]["strategy_type"]
                strategy_counts[strategy] = strategy_counts.get(strategy, 0) + 1

            most_recommended_strategy = max(strategy_counts, key=strategy_counts.get)

            # Market correlation assessment
            bullish_count = sum(
                1 for direction in trend_directions if direction == "bullish"
            )
            bearish_count = sum(
                1 for direction in trend_directions if direction == "bearish"
            )
            sideways_count = sum(
                1 for direction in trend_directions if direction == "sideways"
            )

            market_correlation = (
                "high"
                if max(bullish_count, bearish_count, sideways_count)
                / len(trend_directions)
                > 0.7
                else "medium"
            )
            ...
            analysis["overall_assessment"] = {
                ...
                "market_characteristics": {
                    ...
                    "average_volatility_percentile": round(
                        avg_volatility_percentile, 1
                    ),
                ...
                "portfolio_strategy": {
                    "recommended_approach": most_recommended_strategy,
                    "strategy_distribution": strategy_counts,
                    "diversification_level": (
                        "high"
                        if len(strategy_counts) > 2
                        else "medium" if len(strategy_counts) == 2 else "low"
                    ),
                    "risk_synchronization": (
                        "high" if market_correlation == "high" else "medium"
                    ),
                },
                "portfolio_risk_adjustments": {
                    "overall_position_sizing": (
                        "decrease"
                        if avg_volatility_percentile > 75
                        else (
                            "increase" if avg_volatility_percentile < 25 else "maintain"
                        )
                    ),
                    "correlation_risk": (
                        "high" if market_correlation == "high" else "low"
                    ),
                    "diversification_benefit": (
                        "limited" if market_correlation == "high" else "moderate"
                    ),
                    "max_exposure_per_symbol": f"{min(20, 100 / len(successful_analyses)):.0f}% recommended",
                },
                "market_regime_summary": {
                    "regime_type": f"{dominant_volatility}_vol_{dominant_trend}_trend_{dominant_liquidity}_liquidity",
                    "trading_environment": (
                        "favorable"
                        if (
                            dominant_liquidity in ["high", "medium"]
                            and dominant_trend != "sideways"
                        )
                        else "challenging"
                    ),
                    "optimal_timeframes": list(
                        set(
                            [
                                a["strategy_recommendations"]["primary_recommendation"][
                                    "timeframe"
                                ]
                                for a in successful_analyses
                            ]
                    ...
                    "key_risks": (
                        ...
                        else ["Individual symbol risks"]
                    ),
                },
                "monitoring_priorities": [
                    "Watch for regime changes across correlated symbols",
                    "Monitor liquidity conditions during volatile periods",
                    "Track trend alignment across different timeframes",
                    "Observe volatility spikes that may affect all positions",
                ],
                "portfolio_timing": {
                    "entry_window": (
                        "favorable"
                        if (
                            dominant_liquidity != "low" and dominant_trend != "sideways"
                        )
                        else "wait_for_better_conditions"
                    ),
                    "avoid_periods": [
                        "Low liquidity sessions",
                        "High correlation breakdown periods",
                    ],
                    "optimal_conditions": f"When {dominant_trend} trends are confirmed with {dominant_liquidity} liquidity",
                },
            }

        elif len(successful_analyses) == 1:
            # Single symbol analysis
            single_analysis = successful_analyses[0]
            analysis["overall_assessment"] = {
                "symbols_analyzed": 1,
                "primary_symbol": single_analysis["symbol"],
                "recommended_strategy": single_analysis["strategy_recommendations"][
                    "primary_recommendation"
                ]["strategy_type"],
                "confidence_level": single_analysis["strategy_recommendations"][
                    "primary_recommendation"
                ]["confidence"],
                "key_considerations": single_analysis["monitoring_alerts"][
                    :3
                ],  # Top 3 alerts
            }
            ...
            analysis["overall_assessment"] = {
                "symbols_analyzed": 0,
                "message": "No symbols could be analyzed successfully",
                "recommendation": "Check symbol validity and try again",
            }

        # Add data freshness information
        analysis["data_freshness"] = {
            "volatility_data": f"last_{lookback_days}_days",
            "trend_data": f"last_{lookback_days}_days",
            "liquidity_data": "real_time",
