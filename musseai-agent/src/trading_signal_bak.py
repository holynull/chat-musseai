...
class TradingSignalScheduler:
    ...
    def setup_scheduler(self):
        ...
        self.logger.info(f"Scheduler configured to run every {self.config.execution_interval} minutes")
        self.logger.info(f"Threads will be rebuilt every {self.config.thread_rebuild_hours} hours")
        self.logger.info(f"Max concurrent symbols: {self.config.max_concurrent_symbols}")
        ...
        with self.threads_lock:
            ...
            try:
                ...
                if hasattr(self.sync_client.threads, 'delete'):
                    ...
                    self.logger.info(f"Successfully deleted thread {thread_info.thread_id} for {symbol}")
                else:
                    self.logger.warning(f"Thread deletion not supported by client, marking as obsolete: {thread_info.thread_id}")
                ...
                self.logger.error(f"Failed to delete thread {thread_info.thread_id} for {symbol}: {e}")
        ...
        with self.threads_lock:
            ...
            if symbol in self.threads:
                ...
                if not self.is_thread_expired(thread_info):
                    ...
                    self.logger.debug(f"Using existing thread for {symbol}: {thread_info.thread_id}")
    ...
    def cleanup_all_threads(self):
        ...
        self.logger.info(f"Thread cleanup completed. Cleaned up {len(symbols_to_cleanup)} threads")
    ...
    def get_thread_status_summary(self) -> Dict[str, Any]:
        ...
        with self.threads_lock:
            ...
            for symbol, thread_info in self.threads.items():
                age_hours = (current_time - thread_info.created_at).total_seconds() / 3600
    ...
    async def execute_signal_for_symbol_async(self, symbol: str, retries: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Starting signal generation for {symbol} using thread {thread_id}")
            ...
            async for chunk in chunks:
                self.logger.debug(f"{symbol} - Event: {chunk.event}")
                if hasattr(chunk, 'data') and chunk.data:
                    result_count += 1
                    # Log important events, but avoid flooding logs
                    if chunk.event in ['on_chat_model_end', 'on_tool_end']:
                        self.logger.info(f"{symbol} - {chunk.event}: {str(chunk.data)[:200]}...")
                        
            result["status"] = "SUCCESS"
            result["end_time"] = datetime.now()
            self.logger.info(f"Successfully processed {result_count} events for {symbol}")
            
        except Exception as e:
            result["error"] = str(e)
            result["end_time"] = datetime.now()
            self.logger.error(f"Error executing signal for {symbol} (attempt {retries + 1}): {e}")
            
            # If thread-related error, try to rebuild thread
            if "thread" in str(e).lower() and retries == 0:
                self.logger.info(f"Thread error detected for {symbol}, forcing thread rebuild...")
                self.delete_thread(symbol)
            
            if retries < self.config.max_retries:
                self.logger.info(f"Retrying {symbol} in {self.config.retry_delay} seconds...")
    ...
    async def execute_symbols_concurrently(self, symbols: List[str]) -> Dict[str, Dict[str, Any]]:
        """Execute trading signals for multiple symbols concurrently"""
        self.logger.info(f"Starting concurrent execution for {len(symbols)} symbols: {symbols}")
    ...
    async def execute_trading_signals(self):
        ...
        self.logger.info(f"=== Trading Signal Execution Started at {execution_start} ===")
        
        # Log current thread status before execution
        thread_status = self.get_thread_status_summary()
        self.logger.info(f"Current thread status: {thread_status['total_threads']} active threads")
        ...
        for symbol, result in results.items():
            ...
            if result["start_time"] and result["end_time"]:
                symbol_duration = (result["end_time"] - result["start_time"]).total_seconds()
                self.logger.info(f"{symbol}: {status} (duration: {symbol_duration:.2f}s, thread: {result.get('thread_id', 'N/A')})")
            else:
                self.logger.info(f"{symbol}: {status} (thread: {result.get('thread_id', 'N/A')})")
        ...
        self.logger.info(f"Results: {success_count}/{total_symbols} successful, {failed_count} failed, {error_count} errors")
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.info(f"Thread rebuild interval: {self.config.thread_rebuild_hours} hours")
            self.logger.info(f"Max concurrent symbols: {self.config.max_concurrent_symbols}")
...
def load_config() -> TradingConfig:
    ...
    config_dict['execution_interval'] = int(os.getenv('EXECUTION_INTERVAL_MINUTES', '15'))
    config_dict['thread_rebuild_hours'] = int(os.getenv('THREAD_REBUILD_HOURS', '24'))
    config_dict['max_concurrent_symbols'] = int(os.getenv('MAX_CONCURRENT_SYMBOLS', '5'))  # NEW
