...
const LinkItem = ({ href, label, className = "" }: { href: string; label: string; className?: string }) => (
	<a
		...
		className={`inline-flex items-center px-3 py-1.5 rounded-md text-xs font-medium bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors dark:bg-blue-900/30 dark:text-blue-300 dark:hover:bg-blue-800/50 ${className}`}
	>
		{label}
	</a>
);
...
const TokenMetadataDisplay = ({ input }: { input: any }) => {
	...
	return responseData && tokenSymbol && (
		<div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden w-full max-w-3xl border border-gray-200 dark:border-gray-700 sm:mt-6 md:mt-8">
			{/* Token Header Section */}
			<div className="flex items-center p-5 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800">
				{token.logo && (
					<div className="mr-4 relative h-16 w-16 flex-shrink-0">
						<Image
							src={token.logo}
							alt={`${token.name} logo`}
							className="rounded-full shadow-sm border border-gray-200 dark:border-gray-700"
							width={64}
							height={64}
						/>
					</div>
				...
				<div className="flex-1">
					<div className="flex items-center">
						...
						<span className="ml-2 px-2.5 py-0.5 rounded-full text-sm font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">
							{token.symbol}
						</span>
						{tokensInCategory > 1 && (
					...
					<div className="flex items-center mt-1">
						<span className="text-sm text-gray-500 dark:text-gray-400 mr-3">{token.category}</span>
						{token.platform && (
			...
			<div className="p-5">
				...
				{activeTab === "overview" && (
					<div className="space-y-5">
						...
						{/* Quick Links */}
						<div>
							<h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Quick Links</h3>
							<div className="flex flex-wrap gap-2">
				...
				{activeTab === "details" && (
					<div className="space-y-5">
						...
						{token.contract_address && token.contract_address.length > 0 && (
							<div>
								<h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Contract Addresses</h3>
								<div className="space-y-2 bg-gray-50 dark:bg-gray-900 rounded-lg p-3">
									{token.contract_address.map((contract, index) => (
										<div key={index} className="flex flex-col md:flex-row md:items-center py-2 border-b border-gray-100 dark:border-gray-800 last:border-0">
											<span className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300 px-2 py-1 rounded mr-2 mb-1 md:mb-0 inline-block">
						...
						{/* Token Economics */}
						<div>
							<h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Token Economics</h3>
							<div className="grid grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900 rounded-lg p-3">
								...
								<div className="border-r border-gray-200 dark:border-gray-700 pr-3">
									<div className="text-xs text-gray-500 dark:text-gray-400">Infinite Supply</div>
									<div className="text-sm font-medium text-gray-800 dark:text-gray-200 mt-1"> {token.infinite_supply ? 'Yes' : 'No'} </div> </div>
								<div>
									<div className="text-xs text-gray-500 dark:text-gray-400">Launch Date</div>
									<div className="text-sm font-medium text-gray-800 dark:text-gray-200 mt-1">
										{formatDate(token.date_launched || token.date_added)}
						...
						{token.notice && (
							<div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-100 dark:border-yellow-900/50 rounded-lg p-3">
								<h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-500 mb-1">Notice</h3>
								<p className="text-xs text-yellow-700 dark:text-yellow-400">{token.notice}</p>
							</div>
						)}
					</div>
				)}
				...
				{activeTab === "links" && (
					<div className="space-y-5">
						{/* Official Links */}
						<div>
							<h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Official Links</h3>
						...
						{token.urls.explorer && token.urls.explorer.length > 0 && (
							<div>
								<h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Block Explorers</h3>
								<div className="space-y-2">
									{token.urls.explorer.map((explorer, index) => (
										<a
											...
											target="_blank"
											rel="noopener noreferrer"
											className="block text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 underline-offset-2 hover:underline"
										>
