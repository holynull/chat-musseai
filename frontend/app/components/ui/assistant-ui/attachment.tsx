...
const useAttachmentSrc = () => {
	const { file, src } = useAttachment(
		useShallow((a): { file?: File; src?: string } => {
			...
			const imageContent = a.content?.filter((c) => c.type === "image")[0] as ImageContentPart | undefined;
			if (!imageContent) return {};
			return { src: (imageContent as any)?.src }; // Ensure type safety or update the type definition
		}),
	);
